...
df_input = pd.read_csv('data2.csv', on_bad_lines='skip', header=None)
df_output = pd.DataFrame();

columns_to_transfer = [0,1,2,3,4,5,15,7,8,9,10,11,12,13,16]
...
df_output.insert(20, 'COMMENT', '');


# Set defaults for remaining configs and banks
for idx, column in enumerate(adms10_columns[20: 50]):
    # TODO: Remove hardcoding
    push_column = 20 + idx

    df_output.insert(push_column, 'BANK'+str(idx), 'OFF')
...
# Export to csv
renamed_df_output.to_csv('output.csv', index=False)
